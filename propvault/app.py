...
# 5. Feed
if not bets:
    st.markdown('<div style="text-align:center; padding:40px; color:#475569;">No edges found in the chaos. Check back soon.</div>', unsafe_allow_html=True)
else:
    ...
    for i, b in enumerate(sorted_bets):
        ...
        l5_html = f'<div style="color:#7dd3fc; font-size:12px; font-weight:800; margin-bottom:8px;">{b.get("L5")}</div>' if b.get("L5") else ""
        
        if i == 0: # Lone Unicorn Styling
            cards_html += f"""
            <div class="card" style="border: 2px solid #7dd3fc; background: linear-gradient(145deg, rgba(125, 211, 252, 0.1) 0%, rgba(6, 9, 18, 0.5) 100%); margin-bottom: 40px; position: relative; overflow: hidden;">
                ...
                <div style="display: flex; justify-content: space-between; position: relative; z-index: 1;">
                    <div>
                        <div style="background: #7dd3fc; color: #060912; padding: 2px 10px; border-radius: 100px; font-size: 11px; font-weight: 900; display: inline-block; margin-bottom: 12px;">THE LONE UNICORN</div>
                        ...
                        {l5_html}
                        <span class="strategy-badge {b_theme}" style="font-size: 18px; padding: 4px 12px;">{b_side} {b.get('Market')}</span>
                        <div style="margin-top: 15px; display: flex; gap: 20px;">
                             <div><div style="font-size: 10px; color: #7dd3fc;">NOVIG</div><div style="font-weight: 900; font-size: 20px;">{b.get('Target Odds')}</div></div>
                    ...
                    <div style="text-align: right; align-self: center;">
                        <div style="color: #7dd3fc; font-size: 64px; font-weight: 900;">+{b.get('EV %')}%</div>
                        <div style="font-size: 11px; color: #64748b; font-weight: 800; letter-spacing: 2px;">EDGE</div>
                    </div>
                </div>
            </div>
            """
        else: # Standard List Styling
            cards_html += f"""
            <div class="card" style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    <div style="font-size:24px; font-weight:900;">{b.get('Player')} <span class="strategy-badge {b_theme}" style="font-size:14px; margin-left:10px;">{b_side} {b.get('Market')}</span></div>
