...
# Expanded Prop Markets for NBA and MLB
PROP_MARKETS = [
...
def _fmt_side(name: str, point: float, market_key: str, description: str = "") -> str:
    """Appends player name for props, or formats spreads/totals."""
    # Handle Player/Pitcher/Batter Props
...
def find_ev_bets(api_key: str) -> tuple[list[dict], list[str]]:
    ...
    for sport in SPORTS:
        try:
            ...
            # 2. Player Props
            for event in bulk_events[:15]: # Increased to 15 games
                ...
                for pin_mkt in p_pin["markets"]:
                    ...
                    for p_out in pin_mkt["outcomes"]:
                        # Ensure we match by Player Name (description) AND Outcome (Over/Under) AND Point
